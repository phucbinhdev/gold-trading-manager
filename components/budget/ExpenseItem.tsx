...
export function ExpenseItem({
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex flex-col p-4 rounded-xl border transition-all duration-300 select-none cursor-pointer active:scale-[0.98]",
        expense.is_paid
    ...
    >
      ...
      {expense.note && (
        <div className="mt-2 pl-9 pr-2">
          ...
          {expense.note.split("\n").length > 2 || expense.note.length > 100 ? (
            <button
              onClick={toggleExpand}
              className="text-[10px] text-primary font-semibold mt-1 hover:underline flex items-center gap-1"
            >
              {isExpanded ? (
                <>
