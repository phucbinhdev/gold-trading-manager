...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "soft-surface bg-card text-card-foreground flex flex-col gap-5 rounded-3xl border border-border/55 py-5 shadow-sm",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-base font-bold leading-6", className)}
      {...props}
    />
  )
}
